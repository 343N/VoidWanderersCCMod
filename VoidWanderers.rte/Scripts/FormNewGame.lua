...
function VoidWanderers:FormLoad()
	...
	local ss_modifier = self.PlayerCount > 1 and 0.25 or 0.5
	local max_off_center = 1 / ss_modifier
	for i = 1, max_off_center do
		local a = CreateActor("Fake Brain Case", "VoidWanderers.rte")
		local vert_offset = (self.MidY * i) * ss_modifier
		if a then
			a.Team = Activity.PLAYER_1
			a.HitsMOs = false
			a.GetsHitByMOs = false
			a.Scale = 0
			a.Pos = Vector(self.MidX - i, (self.MidY) - vert_offset + 2)
			a.Health = 1
		...
		if b then
			...
			b.Pos = Vector(self.MidX + i, (self.MidY) + vert_offset - 2)
			b.HitsMOs = false
